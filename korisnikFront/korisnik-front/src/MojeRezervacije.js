...
class MojeRezervacije extends Component{
    ...
    componentDidMount() {
        ...
        axios.get("https://localhost:8300/reservation-service/rezervacije", { headers: { Authorization: `Bearer ${token}` } })
        .then(res=>{
            this.setState({
    ...
    otkazi = (id) =>{
        const token = this.props.user;
        axios.delete("https://localhost:8300/reservation-service/otkazi/" + id, { headers: { Authorization: `Bearer ${token}` } })
            .then(res=>{
